...
**15.** Click the **Cache dependencies** step dropdown. You might notice that **Cache restored successfully** and **Cache restored from key** with the key is the same as step **7** in [11.9 Update Dependency Cache](11-set-up-awsome-books-repository/9-update-cache-dependency). However, the cache key of from **refs/pull/1/merge** pull request also the same. You can check the **last used** attribute to check which cache was used. The **last used** attribute of **main** should be newer than **refs/pull/1/merge**.
